...
function setupStatusHandlers(socket, userConfig) {
    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        try {
            ...
            // 🔒 BOT_MODE protection (works everywhere)
...
if (userConfig.BOT_MODE && participant !== ownerJid) {
    // Optionally notify them (you can remove this block if you want it to stay silent)
...
function setupCommandHandlers(socket, number, userConfig) {
    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        const command = parts[0].toLowerCase();
        const args = parts.slice(1);
  // Prevent others from using commands when bot is in private mode
const ownerJid = `${userConfig.OWNER_NUMBER || sanitizedNumber}@s.whatsapp.net`;
const participant = msg.key.participant || msg.participant || sender; // actual person who sent the message
