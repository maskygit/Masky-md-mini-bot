...
function setupCommandHandlers(socket, number, userConfig) {
    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        const command = parts[0].toLowerCase();
        const args = parts.slice(1);
// 🔒 BOT_MODE protection (final universal fix)
...
// If BOT_MODE is enabled, allow only the owner (in groups or private)
if (userConfig.BOT_MODE) {
    if (participant !== ownerJid && sender !== ownerJid) {
        return; // 🚫 Ignore all messages not from the owner
    }
}
        try {
            switch (command) {
