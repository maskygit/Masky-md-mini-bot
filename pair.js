...
// 🔒 BOT_MODE protection
if (userConfig.BOT_MODE === true || userConfig.BOT_MODE === 'true') {
    const ownerJid = `${userConfig.OWNER_NUMBER}@s.whatsapp.net`;
    if (sender !== ownerJid) {
        await socket.sendMessage(sender, { 
            text: '🚫 Bot is currently in *Private Mode*. Only the owner can use commands.\n\n> © ᴛʜɪꜱ ʙᴏᴛ ʙʏ ɪꜱʀᴇᴀʟ ᴛᴇᴄʜ ᴅᴇᴠ',
            contextInfo: maskyContext
        });
        return; // stop others from using it
    }
}
...
`;
    ...
    const configText = `These are the bot config settings
┌──⫷ 𝐒ʏꜱᴛᴇᴍ 𝐒ᴛᴀᴛᴜꜱ ⫸──┐
${config.PREFIX}autostatus ${viewStatus}
${config.PREFIX}autorecord ${records}
${config.PREFIX}autolike ${likeStatus}
└───────────────────────┘`;
...
case 'mode': {
    ...
    if (!mode || !['on', 'off'].includes(mode)) {
        await socket.sendMessage(sender, {
            text: `⚙️ Usage: *${config.PREFIX}mode on* or *${config.PREFIX}mode off*\n\n> Toggle Private Mode (owner-only access)`,
            contextInfo: maskyContext
        });
        break;
    }

    const isOwner = sender === `${userConfig.OWNER_NUMBER}@s.whatsapp.net`;
    if (!isOwner) {
        await socket.sendMessage(sender, {
    ...
    userConfig.BOT_MODE = (mode === 'on');
    await saveUserConfig(sanitizedNumber, userConfig);

    await socket.sendMessage(sender, {
        text: `✅ Private Mode has been *${mode === 'on' ? 'enabled' : 'disabled'}*.\n\n> Only the owner can use commands when enabled.`,
        contextInfo: maskyContext
    });
    break;
}
