...
`;
    ...
    break;
        ...
        const command = parts[0].toLowerCase();
        const args = parts.slice(1);
// 🔒 BOT_MODE protection (final tested version)
const ownerJid = `${userConfig.OWNER_NUMBER || sanitizedNumber}@s.whatsapp.net`;
const from = msg.key.remoteJid; // The chat where message came from
const participant = msg.key.participant || sender; // Actual message sender
const isGroup = from.endsWith('@g.us');

// If BOT_MODE is enabled (private mode)
if (userConfig.BOT_MODE) {
    // Allow owner everywhere (DM or group)
    if (participant !== ownerJid && from !== ownerJid) {
        return; // 🚫 Ignore message from non-owner users
    }
}
        try {
            switch (command) {
