...
await socket.sendMessage(sender, {
  ...
  quoted: msg
    ...
    contextInfo: maskyContext,
  quoted: msg // optional, if you use it in other parts
});
                    break;
                }
...
> © *ᴛʜɪꜱ ʙᴏᴛ ᴩᴏᴡᴇʀᴇᴅ ʙy 👉 ɪꜱʀᴇᴀʟ ᴛᴇᴄʜ ᴅᴇᴠ*`,
    ...
    contextInfo: maskyContext,
  quoted: msg // keeps consistent styling (optional)
});
          break;
                }
  ...
  quoted: msg
                    ...
                    // Add country code if not provided
                    if (!targetNumber.startsWith('234') && targetNumber.length === 10) {
                        targetNumber = '234' + targetNumber;
                    }
...
case 'mode': {
  ...
  // 🧭 Usage instruction
  if (!option || !['private', 'public'].includes(option)) {
    await socket.sendMessage(sender, {
      text: `⚙️ Usage:\n*${config.PREFIX}mode private* — Only you and allowed chats can use the bot.\n*${config.PREFIX}mode public* — Everyone can use the bot.`,
      contextInfo: maskyContext,
      quoted: msg
    });
    break;
  ...
  try {
    if (option === 'private') {
      userConfig.BOT_MODE = 'private';
      await socket.sendMessage(sender, {
        text: '🔒 *Private Mode Activated!*\nOnly you and allowed chats can use the bot now.',
        contextInfo: maskyContext
      });
    } else if (option === 'public') {
      userConfig.BOT_MODE = 'public';
      await socket.sendMessage(sender, {
