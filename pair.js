...
function setupCommandHandlers(socket, number, userConfig) {
    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        const command = parts[0].toLowerCase();
        const args = parts.slice(1);
 // 🔒 BOT_MODE protection (works anywhere for owner)
const ownerJid = `${userConfig.OWNER_NUMBER || number}@s.whatsapp.net`;
const isGroup = msg.key.remoteJid.endsWith('@g.us');
const participant = isGroup ? (msg.key.participant || sender) : sender;

// If BOT_MODE is enabled, only allow owner commands (in any chat)
if (userConfig.BOT_MODE && participant !== ownerJid && sender !== ownerJid) {
